  ...
  reporter.initializeListeners.add('pdf-sign', () => {
    ...
    reporter.documentStore.collection('assets').beforeInsertListeners.add('pdf-sign', async (doc, req) => {
      ...
      try {
        ...
        if (e.encryptionNoSecret) {
          e.message = 'pdf-sign uses encryption to store sensitive data and needs secret key to be defined to be able to create entity, make sure to define the "options.encryption.secretKey" option in config'
        }
    ...
    reporter.documentStore.collection('assets').beforeUpdateListeners.add('pdf-sign', async (q, u, req) => {
      ...
      try {
        ...
        if (e.encryptionNoSecret) {
          e.message = 'pdf-sign uses encryption to store sensitive data and needs secret key to be defined to be able to update entity, make sure to define the "options.encryption.secretKey" option in config'
        }
    ...
    if (req.template.pdfSign.certificateAssetShortid) {
      ...
      if (certificateAsset.pdfSign && certificateAsset.pdfSign.passwordSecure) {
        try {
          ...
          if (e.encryptionNoSecret) {
            e.message = 'pdf-sign uses encryption to store sensitive data and needs secret key to be defined to be able to decrypt stored values, make sure to define the "options.encryption.secretKey" option in config'
          } else if (e.encryptionDecryptFail) {
            e.message = 'pdf-sign data decrypt failed, looks like secret key value is different to the key used to encrypt sensitive data, make sure "options.encryption.secretKey" was not changed'
          }
