  ...
  reporter.initializeListeners.add('pdf-sign', () => {
    ...
    reporter.documentStore.collection('assets').beforeUpdateListeners.add('pdf-sign', async (q, u, req) => {
      ...
      try {
        ...
        if (e.encryptionNoSecret) {
          e.message = const missingSecretMessage = 'pdf-sign extension uses encryption to store sensitive data and needs secret key to be defined. Please fill "encryption.secretKey" at the root of the config or disable encryption using "encryption.enabled=false".'
