...
module.exports = function (reporter, definition) {
  ...
  if (reporter.compilation) {
    reporter.compilation.include('scriptPdfSign', path.join(__dirname, 'scriptPdfSign.js'))    
  }
  ...
  reporter.afterRenderListeners.insert({ after: 'pdf-utils', before: 'scripts' }, 'pdfSign', async (req, res) => {
    ...
    const result = await reporter.executeScript({
      ...
      execModulePath: reporter.execution ? reporter.execution.resolve('scriptPdfSign') : path.join(__dirname, 'scriptPdfSign.js'),        
    }, req)
