...
module.exports = function (reporter, definition) {
  ...
  reporter.initializeListeners.add('pdf-sign', () => {
    ...
    reporter.documentStore.collection('assets').beforeInsertListeners.add('pdf-sign', (doc, req) => {
      ...
      doc.pdfSign.passwordSecure = crypto.createCipheriv(doc.pdfSign.passwordEncryption, definition.options.secret, iv)
    ...
    reporter.documentStore.collection('assets').beforeUpdateListeners.add('pdf-sign', (q, u, req) => {
      ...
      const cypher = crypto.createCipheriv(u.$set.pdfSign.passwordEncryption, definition.options.secret, iv)
      const encrypted = cypher.update(u.$set.pdfSign.passwordRaw, 'utf8', 'base64')
      u.$set.pdfSign.passwordSecure = encrypted + cypher.final('base64')
  ...
  reporter.afterRenderListeners.insert({ after: 'pdf-utils', before: 'scripts' }, 'pdfSign', async (req, res) => {
    ...
    let password = req.template.pdfSign.password
    ...
    if (req.template.pdfSign.certificateAssetShortid) {
      ...
      if (certificateAsset.pdfSign && certificateAsset.pdfSign.passwordSecure) {
        password = crypto.createDecipheriv(certificateAsset.passwordEncryption || 'aes-256-ctr', definition.options.secret, iv)
          .update(certificateAsset.pdfSign.passwordSecure, 'base64', 'utf8')
      }
    } else {
