...
function addSignaturePlaceholder (pdfBuffer, req) {
  ...
  sig.prop('Reason', new PDF.String(req.template.pdfSign.reason || 'Signed'))
  // 'D:20190521082633Z')
...
module.exports = async (pdfBuffer, certificateBuffer, password, req) => {
  pdfBuffer = await addSignaturePlaceholder(pdfBuffer, req)

  const signer = new SignPdf()
  return signer.sign(pdfBuffer, certificateBuffer, { passphrase: password })
}
